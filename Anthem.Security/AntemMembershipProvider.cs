...
{
    public sealed class AntemMembershipProvider : MembershipProvider
    {
        #region Private
        // Global connection string, generated password length, generic exception message, event log info.
        private int newPasswordLength = 8;
        private string eventSource = "AntemMembershipProvider";
        ...
        /// <summary>
        /// Allows isolation and atomicity of data related jobs
        /// </summary>
        [Import]
        public ExportFactory<IUnitOfWork> UnitOfWork { get; set; }

        [Import]
        public IRepository<Entities.Users> Repository { get; set; }
        ...
        private void UpdateFailureCount(string username, string failureType)
        {
            ...
            using (IUnitOfWork unitOfWork = UnitOfWork.CreateExport().Value)
            {
                try
                {
        ...
        private IList<Entities.Users> GetUsers()
        {
            ...
            using (IUnitOfWork unitOfWork = UnitOfWork.CreateExport().Value)
            {
                try
                {
                    usrs = Repository.CreateCriteria()
                          .Add(NHibernate.Criterion.Restrictions.Eq("ApplicationName", this.ApplicationName))
                          .List<Entities.Users>();
                }
                catch (Exception e)
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                using (IUnitOfWork unitOfWork = UnitOfWork.CreateExport().Value)
                {
                    ...
                    try
                    {
                        // Find a way to distinguish rejected and success
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (IUnitOfWork unitOfWork = UnitOfWork.CreateExport().Value)
            {
                try
                {
                    totalRecords = (Int32)Repository.CreateCriteria()
                                    .Add(NHibernate.Criterion.Restrictions.Eq("ApplicationName", this.ApplicationName))
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            using (IUnitOfWork unitOfWork = UnitOfWork.CreateExport().Value)
            {
                try
                {
                    numOnline = (Int32)Repository.CreateCriteria()
                                           .Add(NHibernate.Criterion.Restrictions.Eq("ApplicationName", this.ApplicationName))
